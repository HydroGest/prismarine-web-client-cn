...
class OptionsScreen extends LitElement {
  ...
  render () {
    return html`
      ...
      <main>
        <div class="wrapper">
          <pmui-slider pmui-label="X轴鼠标敏感度" pmui-value="${this.mouseSensitivityX}" pmui-min="1" pmui-max="100" @input=${(e) => {
            this.mouseSensitivityX = Number(e.target.value)
            window.localStorage.setItem('mouseSensX', this.mouseSensitivityX * 0.0001)
          }}></pmui-slider>
          <pmui-slider pmui-label="Y轴鼠标敏感度" pmui-value="${this.mouseSensitivityY}" pmui-min="1" pmui-max="100" @input=${(e) => {
            this.mouseSensitivityY = Number(e.target.value)
            window.localStorage.setItem('mouseSensY', this.mouseSensitivityY * 0.0001)
          }}></pmui-slider>
        </div>
        <div class="wrapper">
          <pmui-slider pmui-label="聊天区宽度" pmui-value="${this.chatWidth}" pmui-min="0" pmui-max="320" pmui-type="px" @input=${(e) => {
            this.chatWidth = Number(e.target.value)
            window.localStorage.setItem('chatWidth', `${this.chatWidth}`)
            document.documentElement.style.setProperty('--chatWidth', `${this.chatWidth}px`)
          }}></pmui-slider>
          <pmui-slider pmui-label="聊天区高度" pmui-value="${this.chatHeight}" pmui-min="0" pmui-max="180" pmui-type="px" @input=${(e) => {
        ...
        <div class="wrapper">
          <pmui-slider pmui-label="聊天区大小" pmui-value="${this.chatScale}" pmui-min="0" pmui-max="100" @input=${(e) => {
            this.chatScale = Number(e.target.value)
            window.localStorage.setItem('chatScale', `${this.chatScale}`)
            document.documentElement.style.setProperty('--chatScale', `${this.chatScale / 100}`)
          }}></pmui-slider>
          <pmui-slider pmui-label="音量" pmui-value="${this.sound}" pmui-min="0" pmui-max="100" @input=${(e) => {
            this.sound = Number(e.target.value)
            window.localStorage.setItem('sound', `${this.sound}`)
          }}></pmui-slider>
        </div>
        <div class="wrapper">
          <pmui-button pmui-width="150px" pmui-label="键位绑定" @pmui-click=${() => displayScreen(this, document.getElementById('keybinds-screen'))}></pmui-button>
          <pmui-slider pmui-label="GUI 大小" pmui-value="${this.guiScale}" pmui-min="1" pmui-max="4" pmui-type="" @input=${(e) => {
...
: html`
          <div class="wrapper">
            <pmui-slider pmui-label="渲染距离" pmui-value="${this.renderDistance}" pmui-min="2" pmui-max="6" pmui-type=" chunks" @input=${(e) => {
              this.renderDistance = Number(e.target.value)
              window.localStorage.setItem('renderDistance', `${this.renderDistance}`)
            }}></pmui-slider>
            <pmui-slider pmui-label="FOV 视场角" pmui-value="${this.fov}" pmui-min="30" pmui-max="110" pmui-type="" @input=${(e) => {
        ...
        <div class="wrapper">
          <pmui-button pmui-width="150px" pmui-label=${'强制移动端控制：' + (this.forceMobileControls ? 'ON' : 'OFF')} @pmui-click=${() => {
        ...
        <pmui-button pmui-width="200px" pmui-label="完成" @pmui-click=${() => displayScreen(this, document.getElementById(this.isInsideWorld ? 'pause-screen' : 'title-screen'))}></pmui-button>
      </main>
    `
  }
}
